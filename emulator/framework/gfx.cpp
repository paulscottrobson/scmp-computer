...
//					SDL2 Rendering version 13 Nov 2017.
...
static int background,wwidth,wheight;
...
void GFXStart(void) {
	...
	while(isRunning) {																// While still running.
		...
		SDL_SetRenderDrawColor(renderer, RED(background),GREEN(background),BLUE(background),255);
		SDL_RenderClear(renderer);
		GFXXRender(renderer);														// Ask app to render state.
		SDL_RenderPresent(renderer);												// And update the main window.
	}
	SDL_CloseAudio();
}
...
void GFXRectangle(SDL_Rect *rc,int colour) {
	SDL_SetRenderDrawColor(renderer, RED(colour),GREEN(colour),BLUE(colour),255);
	...
	for (int x = 0;x < 5;x++) {														// 5 Across
		...
		for (int y = 0;y < 7;y++) {													// 7 Down
			...
			if (fontdata[character*5+x] & (0x01 << y)) {							// Is bit set ? (note inversion)
				SDL_RenderFillRect(renderer,&rc);									// If so, draw the pixel
			}
		}
	}
}
...
void GFXSetCharacterSize(int xSize,int ySize) {
	gridInfo.w = wwidth / xSize / 6;
	gridInfo.h = wheight / ySize / 8;
	gridInfo.w = (gridInfo.w < gridInfo.h) ? gridInfo.w : gridInfo.h;
	gridInfo.h = gridInfo.w;
	gridInfo.x = wwidth / 2 - gridInfo.w * xSize * 6 / 2;
	gridInfo.y = wheight / 2 - gridInfo.h * ySize * 8 / 2;
}
